...
public class TradeController {
	 ...
	 * @return the created trade with HTTP status code
	 */
	@PostMapping("/trade")
	public ResponseEntity<Trade> createNewFxTrade(@RequestBody Trade trade) {
	...
	public ResponseEntity<Map<String, Trade>> confirmNewFxTrade(@PathVariable int tradeId) {
	...
	public ResponseEntity<Trade> findTradeById(@PathVariable int tradeId) {
		...
		if (tradeById != null) {
			...
			return new ResponseEntity<>(tradeById, HttpStatus.NOT_FOUND);
		}
	...
	public ResponseEntity<Map<String, Boolean>> cancelTrade(@PathVariable int tradeId) {
		...
		if (deleted) {
			...
			response.put("Trade Deleted", deleted);
		} else {
			response = new HashMap<>();
